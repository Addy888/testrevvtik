...
//     const result = await generateText({
//       model: anthropic("claude-3-haiku-20240307"),
//       system: `You are an expert-level AI Sales Coach designed for professional sales training.`,
//       prompt: conversationPrompt,
//       maxOutputTokens: 800,
//     })
...
//     try {
//       const suggestionResult = await generateText({
//         model: anthropic("claude-3-haiku-20240307"),
//         system: `
// Generate ONLY 3 short next-step suggestions for a sales trainee.
// - Max 5 words
// - Action oriented
// - Comma separated
// Example:
// Practice mock call, Try objection script, Review closing checklist
// `,
//         prompt: `
// Conversation:
// ${conversationPrompt}
...
//       if (suggestionResult?.text) {
//         suggestions = suggestionResult.text
//           .split(",")
//           .map((s) => s.trim())
//           .filter(Boolean)
//           .slice(0, 3)
//       }
//     } catch {
//       console.warn("Suggestion generation failed")
//     }
...
async function safeAICall(fn: () => Promise<any>, retries = 5): Promise<any> {
  let delay = 1200
  ...
  for (let i = 0; i < retries; i++) {
    try {
      ...
      const retryable =
        err?.statusCode === 529 ||
        err?.responseBody?.error?.type === "overloaded_error" ||
        err?.isRetryable

      if (!retryable) throw err
      if (i === retries - 1) throw err

      console.log("Claude overloaded → retry in", delay)
...
export async function POST(req: Request) {
  try {
    ...
    if (!currentSessionId) {
      ...
      try {
        const titleResult = await safeAICall(() =>
          generateText({
            model: anthropic("claude-3-haiku-20240307"),
            prompt: `Generate a short 4-6 word title for this conversation:\n\n${lastUserMessage}`,
            maxOutputTokens: 20,
        ...
        if (titleResult?.text) {
          title = sanitizeAI(titleResult.text)
        }
      } catch {
    ...
    const result = await safeAICall(() =>
      generateText({
        model: anthropic("claude-3-haiku-20240307"),
        system: `
You are an expert-level AI Sales Coach.

STRICT RULES:
- Never use asterisks (*)
- Never write tone directions
- Never write stage actions
- Never use markdown
...
`,
    ...
    const cleanText = sanitizeAI(result.text)
    ...
    try {
      const suggestionResult = await safeAICall(() =>
        generateText({
          model: anthropic("claude-3-haiku-20240307"),
          system: `
Generate ONLY 3 short next-step suggestions.
Comma separated.
`,
          prompt: `${conversationPrompt}\nAI:${cleanText}`,
          maxOutputTokens: 60,
      ...
      if (suggestionResult?.text) {
        suggestions = sanitizeAI(suggestionResult.text)
          .split(",")
          .map((s: string) => s.trim())
          .filter(Boolean)
          .slice(0, 3)
